...
function findIndexByProperty(arr, name) {
  ...
  return arr.findIndex((index) => index.name === name);
...
function findLastEvenNumber(arr) {
  ...
  return arr.findLast((num) => num % 2 === 0);
}
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  let subArrayString = arr.slice(startIdx).join(" ");
...
function getPositiveArrayValues(arr) {
  ...
  let Array = arr.values((obj) => obj);
  let result = arr.filter((obj) => obj > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  let Array = arr.splice(startIdx);
  let result = arr.map((startIdx) => startIdx * 2);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  let hold = Array.map((element) => element * element);
  // За допомогою методу filter() фільтруємо елементи, які більше за порогове значення.
  let result = hold.filter((element) => element > threshold);
...
function insertElementAt(arr, index, element) {
  ...
  // let insert = arr.splice(index);
  // let result = insert.splice(element);
...
function expandArrayByN(arr, n) {
  ...
  // let expandedArray = arr.flatMap((num) => num);
...
function findLastNegativeNumberIndex(arr) {
  ...
  let result = arr.findLastIndex((index) => index < -1);
